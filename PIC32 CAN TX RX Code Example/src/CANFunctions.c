...
* Compiler:        MPLAB® XC32
*
* Revisions:      6/24/13 R. Wall - TArget Cerebot MX7ck using XC32 1.30
*
*                 October 26, 2014 - R. Wall Added peripheral library
...
DESCRIPTION:    This function initializes CAN1 for standard 11 bit ID
PARAMETER1:     None
RETURN VALUE:   None
Notes:          None
END DESCRIPTION ************************************************************/
 ...
 * uses this buffer memory to store messages to be transmitted and received.
 * The size of the memory buffer should be enough to accommodate the 
 * required number of message buffers and channels.
 *
 * Parameter 1:     CAN Module:  CAN1 or CAN2
 ...
 */
    CANConfigureFilter(CAN1, CAN_FILTER0, CAN_SID_MSG_1, CAN_SID);
 ...
 */
    CANConfigureFilterMask(CAN1, CAN_FILTER_MASK0, SID_FILTER_MASK,
                              CAN_SID, CAN_FILTER_MASK_IDE_TYPE);
...
DESCRIPTION:    This function initializes CAN2 for standard 11 bit ID
PARAMETER1:     None
RETURN VALUE:   None
...
/* Step 4: Configure channel 0  */
 ...
 /* Step 5: Configure filters and mask. */
    ...
    CANConfigureFilter(CAN2, CAN_FILTER0, CAN_SID_MSG_4 , CAN_SID);

    CANConfigureFilterMask(CAN2, CAN_FILTER_MASK0, SID_FILTER_MASK,
                                CAN_SID, CAN_FILTER_MASK_IDE_TYPE);
 ...
 * will indicate if the LEDD should be switched ON or OFF. */
 ...
 * Description:
 *   This function will send a message to CAN2 with SID 0x0202. The data
...
//      message->msgSID.SID	= 0x202;    // Send message to CAN2
        message->msgSID.SID = (WORD) (CAN_SID_MSG_4 & SID_BIT_MASK);

        message->msgEID.IDE	= 0;        // IDE = 0 means SID message.
...
/* Form a Extended ID message. Start by clearing the buffer. */
        ...
        message->msgSID.SID = (WORD) (CAN_SID_MSG_1 & SID_BIT_MASK);
        message->msgEID.IDE = 0;            /* IDE = 0 means SID message. */
        message->msgEID.RTR = 0;            /* Not an RTR message. */
        message->msgEID.DLC = 1;            /* Send one byte of data. */
